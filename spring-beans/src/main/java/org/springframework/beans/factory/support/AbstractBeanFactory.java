...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
			...
			throws BeansException {
		//1.获取真实的BeanName,FactoryBean本身创建的时候，传的是&BeanName，那么就需要通过这个，拿到真实的BeanName。还有一种情况是，当为Bean起别名到时候，也需要通过这个拿到真实的BeanName。
		String beanName =
				transformedBeanName(name); //取指定 alias 所表示的最终 beanName，例如别名 A 指向名称为 B 的 bean 则返回 B; 如果是以&开头，那么就是FactoryBean,去掉开头的&符号
		Object bean;
		...
		//为什么会有这一步呢？第一个是为了解决循环依赖，这个很好理解。第二个是为了不重复创建，比如A依赖了B，C也依赖了B，在创建A的时候，已经把B创建好了，所以，创建C的时候直接从缓存拿不是更高效吗？
		...
		if (sharedInstance != null && args == null) {
			...
			// Fail if we're already creating this bean instance:4.只有在单例情况才会尝试解决循环依赖，原型模式情况下是不支持的
			...
			if (parentBeanFactory != null &&
					!containsBeanDefinition(beanName)) {  //如果父类工厂不为空，并且当前beanDefinitionMap 没有对应的beanName ，那就委托父类处理，
		...
		// Check if required type matches the type of the actual bean instance.检查需要的类型是否符合bean的实际类型，如果不符合，会尝试使用类型转换器转换
